...
The following creates the full database from scratch. This matches what `data/lib/db.ts` generates via `initSchema()`.
...
-- =============================================================
-- trivia_pool — Pre-generated trivia questions for Daily Challenge.
-- 1,235 questions across 7 formats, generated by pipeline scripts.
-- Built by data/scripts/5-trivia/generate-pool.ts + generate-fakes.ts.
-- =============================================================
...
CREATE INDEX idx_trivia_pool_format ON trivia_pool(format);
CREATE INDEX idx_trivia_pool_used   ON trivia_pool(used_date);


-- =============================================================
-- trivia_daily — Daily question sets (lazy-assembled on first request).
-- Backend picks 5 unused questions from trivia_pool on first
-- /trivia/daily request for a date, writes here. No cron needed.
-- =============================================================
...
-- =============================================================
-- trivia_scores — Anonymous player scores for Daily Challenge.
-- Country derived from nginx GeoIP2 X-Country-Code header.
-- =============================================================
CREATE TABLE trivia_scores (
    ...
    answers_json  TEXT,                    -- JSON array of per-question results
    created_at    TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_trivia_scores_date    ON trivia_scores(date);
CREATE INDEX idx_trivia_scores_country ON trivia_scores(country_code);
```
...
npx tsx scripts/1-viator/indexer.ts --full                  # Index all 2,712 leaf destinations (~20 hours)
npx tsx scripts/2-oneliners/backfill-oneliners-batch.ts      # Generate AI one-liners (~14 hours)
npx tsx scripts/3-city-intel/build-city-profiles.ts          # Build city intelligence profiles (~1 hour via Batch API)
npx tsx scripts/3-city-intel/backfill-city-readings.ts <results.jsonl> [...]  # Load batch results
npx tsx scripts/4-chains/generate-chains-v2.ts               # Generate Six Degrees chains (~2 hours via Batch API)
npx tsx scripts/5-trivia/generate-pool.ts                    # Generate trivia pool (1,035 SQL-based questions)
npx tsx scripts/5-trivia/generate-fakes.ts                   # Generate fake tour titles via Haiku (200 real_or_fake)
```
...
The backend API opens the DB in **read-only** mode for most endpoints. Trivia endpoints (score submission, daily assembly) use a separate read-write connection. Scripts (indexer, backfill) open in read-write mode with `busy_timeout(5000)` to handle contention. PM2 runs in fork mode (single process) to avoid SQLite write conflicts.
