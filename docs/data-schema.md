...
### `superlatives`
Schema exists for pre-computed daily superlatives. Currently unused — Phase 3 queries superlatives live from the tours table via `getSuperlative()` / `getAllSuperlatives()` in `db.ts`.
